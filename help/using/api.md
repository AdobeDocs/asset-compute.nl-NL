...
Er wordt verwezen naar binaire URL&#39;s, zoals vooraf ondertekende URL&#39;s van Amazon AWS S3 of Azure Blob Storage SAS-URL&#39;s. Wordt gebruikt voor het lezen van het element `source` (`GET` URLs) en het schrijven van de vertoningen (`PUT` URLs). De klant is verantwoordelijk voor het genereren van deze vooraf ondertekende URL&#39;s.
...
| Naam | Type | Beschrijving | Voorbeeld |
|-------------------|----------|-------------|---------|
| `scale` | `number` | Schaal van het watermerk, tussen `0.0` en `1.0` . `1.0` betekent dat het watermerk zijn originele schaal (1 :1) heeft en de lagere waarden verminderen de grootte van het watermerk. | De waarde `0.5` betekent de helft van de oorspronkelijke grootte. |
