...
Nadat elke uitvoering is gemaakt en opgeslagen in een bestand met het pad dat wordt geleverd door `rendition.path`, uploadt de [Asset Compute SDK](https://github.com/adobe/asset-compute-sdk#adobe-asset-compute-worker-sdk) elke uitvoering naar een cloudopslag (AWS of Azure).Een aangepaste toepassing krijgt meerdere uitvoeringen tegelijkertijd als en alleen als de binnenkomende aanvraag meerdere uitvoeringen bevat die verwijzen naar dezelfde toepassings-URL. De upload naar Cloud Storage gebeurt na elke uitvoering en vóór het uitvoeren van de callback voor de volgende uitvoering.

`batchWorker()` heeft een ander gedrag. Alle uitvoeringen worden verwerkt en pas nadat ze zijn verwerkt, worden ze geüpload.
...
## Ontvangen [!DNL Adobe I/O Events] {#receive-aio-events}

De klant peilt het Adobe [!DNL I/O Events]-journaal volgens zijn consumptielogica.De oorspronkelijke URL van het journaal is de URL die wordt opgegeven in de `/register` API-reactie.Gebeurtenissen kunnen worden geïdentificeerd met de `requestId` die aanwezig is in de gebeurtenissen en dezelfde is als geretourneerd in `/process`.Elke vertoning heeft een aparte gebeurtenis die wordt verzonden zodra de vertoning is geüpload (of mislukt). Wanneer de client een overeenkomende gebeurtenis ontvangt, kan de client de resulterende uitvoeringen weergeven of op een andere manier afhandelen.
...
Voor details op hoe te om dagboekgebeurtenissen te krijgen, zie Adobe [[!DNL I/O Events]  API &#x200B;](https://developer.adobe.com/events/docs/guides/api/journaling-api#).
